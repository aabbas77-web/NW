...
uses
    Windows, SysUtils, Classes, RBroker, RFormat;

type
    TLengthyThread = class(TThread)
    private
    ...
    TServerObjectTHREAD = class(TServerObject)
    private
        FWorkerThread : TLengthyThread;
        procedure ThreadJobDone(Sender : TObject);
    public
        procedure Execute; override;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TLengthyThread.Execute;
begin
    Sleep(5000);                 { Wait for 5 secs }
    FResponseBuffer.WriteFields(FALSE, ['Thread job is done (5 seconds sleep)']);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerObjectTHREAD.ThreadJobDone(Sender : TObject);
begin
    FResponseBuffer.WriteFields(TRUE, ['Worker Thread terminated']);
    FResultStatus := 200;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerObjectTHREAD.Execute;
begin
    FWorkerThread                 := TLengthyThread.Create(TRUE);
