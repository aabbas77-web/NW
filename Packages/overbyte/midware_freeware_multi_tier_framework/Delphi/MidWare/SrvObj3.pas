...
Jul 08, 1998  V1.03 Simplified code because WriteFields now support TField
...
const
    VersionServerObjectGETCLIENT = 103;

type
    TServerObjectGETCLIENT = class(TServerObject)
    public
...
procedure Register;
begin
    RegisterComponents('FPiette', [TServerObjectGETCLIENT]);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerObjectGETCLIENT.Execute;
var
...
begin
    ...
    if UData.CliTable.GotoKey then begin
        ...
        for Fld := 0 to UData.CliTable.FieldCount - 1 do
            FResponseBuffer.WriteFields(FALSE, [UData.CliTable.Fields[Fld]]);
    end
    else begin
        { Data not found }
        FResultStatus := 400;
