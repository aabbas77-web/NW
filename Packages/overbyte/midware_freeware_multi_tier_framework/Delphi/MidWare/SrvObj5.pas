...
type
    TServerObjectGETFILE = class(TServerObject)
    public
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerObjectGETFILE.Execute;
var
    Data     : TFileStream;
    FileName : String;
begin
    ...
    if FileName = '' then begin
        FResponseBuffer.WriteFields(TRUE, ['Missing file name']);
        FResultStatus := 400;
        Finish;
    end;
    ...
    try
        ...
        Data.Seek(0, 0);
        FResponseBuffer.WriteFields(TRUE, [FileName]);
        FResponseBuffer.WriteStreamField(FALSE, mwString, Data);
        Data.Destroy;
        FResultStatus := 200;
    except
        on E:Exception do begin
            FResponseBuffer.WriteFields(TRUE, ['Error',
                                               E.Message]);
            FResultStatus := 400;
        end;
    end;
