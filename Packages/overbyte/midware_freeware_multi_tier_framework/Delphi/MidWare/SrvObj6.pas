...
Description:  Example of server object.
              Part of the SrvTst sample application server.
Creation:     Jul 11, 1998
Version:      1.00
...
const
    VersionServerObjectGETIMAGE = 100;

type
    TSObjGETIMAGE = class(TServerObject)
    public
...
procedure Register;
begin
    RegisterComponents('FPiette', [TSObjGETIMAGE]);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSObjGETIMAGE.Execute;
var
...
begin
    ...
    UData.ImgTable.Active := TRUE;

    Key := UpperCase(FRequestBuffer.Fields[0]);
    UData.ImgTable.IndexName := 'BYNAME';
    UData.ImgTable.SetKey;
    UData.ImgTable.FieldByName('LASTNAME').AsString := Key;
    if UData.ImgTable.GotoKey then begin
        { Data found }
        FResultStatus := 200;
        { Copy all fields from the table to the response set }
        for Fld := 0 to UData.ImgTable.FieldCount - 1 do
            FResponseBuffer.WriteFields(FALSE, [UData.ImgTable.Fields[Fld]]);
    end
    else begin
        { Data not found }
        FResultStatus := 400;
