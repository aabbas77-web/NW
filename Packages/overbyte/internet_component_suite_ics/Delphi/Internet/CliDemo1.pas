...
type
  ...
  private
    ...
    procedure CliSocketTestSessionConnected(Sender: TObject; Error: Word);
    procedure Timer1TestTimer(Sender: TObject);
    procedure Timer1TestTimer2(Sender: TObject);
  public
...
begin
    ...
    Count := 0;
    CliSocket.OnSessionConnected := CliSocketTestSessionConnected;
    CliSocket.Proto    := 'tcp';
    CliSocket.Port     := PortEdit.Text;
    CliSocket.Addr     := ServerEdit.Text;
    CliSocket.LineMode := TRUE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TClientForm.CliSocketTestSessionConnected(
    Sender : TObject;
    Error  : Word);
begin
    ConnectError := Error;
    if Error <> 0 then
        ...
        Inc(Count);
        CliSocket.SendStr(SendEdit.Text + ' ' + IntToStr(Count) + #13#10);
        Display(SendEdit.Text + ' ' + IntToStr(Count) + #13#10);
        Timer1.OnTimer  := Timer1TestTimer;
        Timer1.Interval := 500;
        Timer1.Enabled  := TRUE;
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TClientForm.Timer1TestTimer(Sender: TObject);
begin
    Timer1.Enabled  := FALSE;
    Timer1.OnTimer  := Timer1TestTimer2;
    Timer1.Interval := 500;
    Timer1.Enabled  := TRUE;
    CliSocket.Close;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TClientForm.Timer1TestTimer2(Sender: TObject);
begin
    Timer1.Enabled  := FALSE;
    CliSocket.Proto    := 'tcp';
    CliSocket.Port     := PortEdit.Text;
